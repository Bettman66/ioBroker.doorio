...
function BaresipClient(adapter) {
    ...
    function send2Server(id, state) {
        ...
        var dp = (id.split('.'));
        switch (id) {
            case (adapter.config.ring1, dp[0] + '.' + dp[1] + '.ring1button'):
                ...
                break;
            case (adapter.config.ring2, dp[0] + '.' + dp[1] + '.ring2button'):
                ...
                break;
            case (adapter.config.ring3, dp[0] + '.' + dp[1] + '.ring3button'):
                ...
                break;
            case (adapter.config.ring4, dp[0] + '.' + dp[1] + '.ring4button'):
                ...
                switch (jsonContent.type) {
                    ...
                    case ('CALL_DTMF_START'):
                        ...
                        if (dtmf == adapter.config.dtmf1) {
                            ...
                            tout = setTimeout(() => {
                                tout = null;
                                if (adapter.config.out1) { adapter.setForeignState(adapter.config.out1, false, true); }
                            ...
                            tout = setTimeout(() => {
                                tout = null;
                                if (adapter.config.out2) { adapter.setForeignState(adapter.config.out2, false, true); }
                            ...
                            tout = setTimeout(() => {
                                tout = null;
                                if (adapter.config.out3) { adapter.setForeignState(adapter.config.out3, false, true); }
                            ...
                            tout = setTimeout(() => {
                                tout = null;
                                if (adapter.config.out4) { adapter.setForeignState(adapter.config.out4, false, true); }
                    ...
                    case ('CALL_ESTABLISHED'):
                        ...
                        if (adapter.config.call_active) { adapter.setForeignState(adapter.config.call_active, true, true); }
                        adapter.setState('CALL_ACTIVE', true, true);
                        if (adapter.config.call_established) { adapter.setForeignState(adapter.config.call_established, true, true); }
                        adapter.setState('CALL_ESTABLISHED', true, true);
                        adapter.log.debug('CALL_ESTABLISHED');
                        clearTimeout(tout1);
                        tout1 = setTimeout(() => {
                            tout1 = null;
                            if (adapter.config.call_established) { adapter.setForeignState(adapter.config.call_established, false, true); }
                    ...
                    case ('CALL_CLOSED'):
                        ...
                        if (adapter.config.call_active) { adapter.setForeignState(adapter.config.call_active, false, true); }
                        adapter.setState('CALL_ACTIVE', false, true);
                        if (adapter.config.call_closed) { adapter.setForeignState(adapter.config.call_closed, true, true); }
                        ...
                        tout2 = setTimeout(() => {
                            tout2 = null;
                            if (adapter.config.call_closed) { adapter.setForeignState(adapter.config.call_closed, false, true); }
