...
/*                        adapter.setState('CALLING_NUMBER', callerID, function(){
                            adapter.getState('whitelist.EVERY_NUMBER', function (err, state) {
                                if (state.val) {
                                    adapter.log.debug('Whitelist 1: Every Number');
                                    adapter.log.debug('picking up call');
                                    client.write(Netstring.write('{"command":"accept","params":""}'));
                                } else {
                                    adapter.getState('whitelist.EVERY_NUMBER', function (err, state) {
                                    let wlnum1 = state.val;


                                          client.write(Netstring.write('{"command":"dial","params":"' + adapter.config.number4 + '"}'))
                                        };
                                    });
                                //do something when the new state is set
                            });
                        });
*/
                        besch = true;
                        adapter.getState('whitelist.EVERY_NUMBER', (err, state) => {
                            inWL = state.val;
                            adapter.log.debug('inWL Callback:' + inWL);
                            besch = false;
                        ...
                        while (besch === true) {
                            dummy++;
                            if(dummy > 100000000){besch = false;}
                        }

                        //const state = await adapter.getStateAsync('whitelist.EVERY_NUMBER');
                        //var inWL = state.val;
                        //var inWL = adapter.getState('whitelist.EVERY_NUMBER').val;
                        adapter.log.debug('inWL:' + inWL);

                        if (inWL){
                          adapter.log.debug('Whitelist 1: Every Number');
                        }else{
                          adapter.log.debug('Whitelist: Special Numbers');
                        }
                        adapter.getState('whitelist.NUMBER_1', (err, state) => {
                            WLnum1 = state.val;
                        });

//                        const state1 = await adapter.getStateAsync('whitelist.EVERY_NUMBER');
//                        var stateswl = state1.val:
                        //var stateswl = adapter.getState('whitelist.NUMBER_1').val;
                        if (callerID.indexOf(WLnum1) >= 0 && WLnum1.length > 0) {
                          inWL = true;
                          adapter.log.debug('Whitelist Number 1 OK:' + stateswl);
                        }else{
                          adapter.log.debug('Whitelist Number 1 not OK:' + stateswl);
                        }
                        stateswl = adapter.getState('whitelist.NUMBER_2').val;
                        if (callerID.indexOf(stateswl) >= 0 && stateswl.length > 0) {
                          inWL = true;
                          adapter.log.debug('Whitelist Number 2 OK:' + stateswl);
                        }else{
                          adapter.log.debug('Whitelist Number 2 not OK:' + stateswl);
                        }
                        stateswl = adapter.getState('whitelist.NUMBER_3').val;
                        if (callerID.indexOf(stateswl) >= 0 && stateswl.length > 0) {
                          inWL = true;
                          adapter.log.debug('Whitelist Number 3 OK:' + stateswl);
                        }else{
                          adapter.log.debug('Whitelist Number 3 not OK:' + stateswl);
                        }
                        stateswl = adapter.getState('whitelist.NUMBER_4').val;
                        if (callerID.indexOf(stateswl) >= 0 && stateswl.length > 0) {
                          inWL = true;
                          adapter.log.debug('Whitelist Number 4 OK:' + stateswl);
                        }else{
                          adapter.log.debug('Whitelist Number 4 not OK:' + stateswl);
                        }
                        if (inWL){
                            adapter.log.debug('picking up call');
                            client.write(Netstring.write('{"command":"accept","params":""}'));
